---
title: Module's Requirements
---

## Module Requirements
The following sections document the requirements that the this module need to fulfills to .....

| **Requirement Description** | **Measure of<br> Threshold** | **Target<br>Measure** |**Stretch<br>Requirement<br>(Y-N)**|
|-----------------------------| ----------------- | ----------------- | :-----: |
| Surface mounted, 3.3V switching power regulatore | 3.2 Volts | 3.3 Volts | No |
| Surface mounted microcontroller | ESP32 | ESP32 | No |
| Wireless Communication | Able to receive data over bluetooth | Send and receive bluetooth data | No |
| UART bus communication | Able to route, send and recieve messages through shared UART bus | Able to route, send and recieve messages through shared UART bus | No |
| Power sharing | Can recieve power from a pluggable terminal or pin header | Recieves power through the shared UART bus | Yes |
| Central control | Relays human inputs recieved from A1 to necessary subsystems | Translates human inputs recieved from A1 to commands for necessary subsystems with PID | No |
| Debugging | Has LEDs and test pads for debugging | Has LEDs and test pads for debugging | No |
| Backup controls | Has simple on/off switches for power cycling the robot without bluetooth | Has full control capabilities for testing the robot without bluetooth | Yes

